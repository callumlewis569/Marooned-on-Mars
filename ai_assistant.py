...
status = [
            {"item": "oxygen", "safe_level": 20, "current_level": oxygen, "solution if not in inventory": "Plant more trees", "safety": "safe" if oxygen >= 20 else "not safe" }, 
            {"item": "thirst", "safe_level": 20, "current_level": thirst, "solution if not in inventory": "Find ice blocks and bring them back to the spaceship", "safety": "safe" if thirst >= 20 else "not safe"},  
            {"item": "hunger", "safe_level": 40, "current_level": hunger, "solution if not in inventory": "Plant more plants and harvest them", "safety": "safe" if hunger >= 40 else "not safe"},
            {"item": "character fuel", "safe_level": 10, "current_level": character_fuel, "solution if not in inventory": "Mine fuels", "safety": "safe" if character_fuel >= 10 else "not safe"},
            {"item": "ship fuel", "safe_level": 10, "current_level": ship_fuel, "solution if not in inventory": "Mine fuels", "safety": "safe" if ship_fuel >= 10 else "not safe"}
            ]
...
fuels = [
            {"name": "combustite", "number": combustite, "energy": 30},
            {"name": "ionflux", "number": ionflux, "energy": 60},
            {"name": "void ether", "number": void_ether, "energy": 90}
            ]

inventory = [ores, plants, radioactive, general, fuels]
...
async def good():
    ...
    agent_id = client.agents.create(
        ...
        Check the statuses and inventory'''+ str(status)+ str(inventory)+ 
